...
export async function projectDiagnostics(projects) {
  ...
  for (const project of projects.filter((item) => item.folder).slice(0, 40)) {
    ...
    if (!(await exists(folder))) {
      results.push({ projectId: project.id, folder, exists: false });
      continue;
    }
    let git = null;
    try {
      const [status, log, branch] = await Promise.all([
        execute("git", ["-C", folder, "status", "--porcelain"], {
          windowsHide: true,
          timeout: 5000,
          maxBuffer: 1024 * 1024,
        }),
        execute(
          "git",
          ["-C", folder, "log", "-1", "--format=%h%x00%s%x00%cI"],
          { windowsHide: true, timeout: 5000, maxBuffer: 1024 * 1024 },
        ),
        execute("git", ["-C", folder, "branch", "--show-current"], {
          windowsHide: true,
          timeout: 5000,
        }),
      ]);
      ...
      git = {
        ...
        lastCommit: hash ? { hash, message, date } : null,
      };
    } catch {
      /* The project folder does not need to be a Git repository. */
    }
...
export function markdownWorklog(data, { from, to } = {}) {
  const lower = from || "0000-00-00";
  const upper = to || "9999-99-99";
  const logs = data.worklogs
    .filter((item) => item.date >= lower && item.date <= upper)
  ...
  const lines = [
    ...
    `期間: ${from || "すべて"} 〜 ${to || "現在"}`,
    "",
    `合計: ${logs.reduce((sum, item) => sum + (item.minutes ?? 0), 0)}分`,
    "",
  ];
  for (const [date, items] of grouped) {
    ...
    for (const item of items)
      lines.push(
        `- ${item.title}${item.minutes ? ` (${item.minutes}分)` : ""}${item.projectId ? ` — ${projects.get(item.projectId) ?? "不明なプロジェクト"}` : ""}${item.body ? `\n  ${item.body.replace(/\n/g, "\n  ")}` : ""}`,
      );
    lines.push("");
  }
  return `${lines.join("\n")}\n`;
...
export function productivityInsights(data) {
  ...
  for (let offset = 13; offset >= 0; offset -= 1) {
    ...
    const minutes = data.worklogs
      .filter((item) => item.date === key)
      .reduce((sum, item) => sum + (item.minutes ?? 0), 0);
    const completed = data.tasks.filter(
      (item) => item.completedAt && localDay(item.completedAt) === key,
    ).length;
  ...
  const recordedDays = new Set([
    ...
    ...data.tasks
      ...
      .map((item) => localDay(item.completedAt)),
  ]);
  const cursor = new Date(today);
  if (!recordedDays.has(localDay(cursor))) cursor.setDate(cursor.getDate() - 1);
  while (recordedDays.has(localDay(cursor))) {
    streak++;
  ...
  for (const log of data.worklogs)
    if (log.projectId)
      byProject.set(
        log.projectId,
        (byProject.get(log.projectId) ?? 0) + (log.minutes ?? 0),
      );
  return {
    days,
    streak,
