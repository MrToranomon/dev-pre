...
function isTextFile(name) {
  return (
    ...
    ["readme", "license", "makefile", "dockerfile"].includes(
...
async function extractPdf(file) {
  ...
  try {
    for (
      let pageNumber = 1;
      pageNumber <= Math.min(document.numPages, 500);
      pageNumber += 1
    ) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          files.push({
            ...
            extension: path.extname(entry.name).toLowerCase() || "file",
          });
        } catch (error) {
          errors.push({ path: location, error: error.message });
        }
      }
    }
...
export class SearchEngine {
  ...
  async init() {
    if (await exists(this.file)) {
      try {
        ...
        if (Array.isArray(cached.documents)) {
          this.index = cached.documents;
          this.status = { ...this.status, ...cached.status, state: "ready" };
        }
      } catch {
        /* A fresh index is safer than trusting damaged cached data. */
      }
    }
    return this;
  }
  ...
  async build(settings) {
    ...
    this.running = this.#build(settings)
      .catch((error) => {
  ...
  async #build(settings) {
    ...
    const walked = await walkFiles(roots, {
      maxFiles: settings.searchMaxFiles,
      onProgress: (files) => {
    ...
    for (const item of walked.files) {
      ...
      if (
        ...
        if (cached.content) extracted += 1;
        continue;
      }
      let content = "";
      let extractionError = null;
      if (
        item.size <= settings.searchMaxFileBytes &&
        (isTextFile(item.name) || [".pdf", ".docx"].includes(item.extension))
      ) {
        try {
          content = (await extractContent(item.path, item.size)).slice(
            0,
            2_000_000,
          );
          extracted += 1;
      ...
      this.status.files = documents.length;
      this.status.contentFiles = extracted;
    }
    this.index = documents;
    this.status = {
      state: "ready",
      files: documents.length,
      contentFiles: extracted,
      errors:
        walked.errors.length +
        documents.filter((item) => item.extractionError).length,
      updatedAt: new Date().toISOString(),
      limited: walked.limited,
      roots,
    };
  ...
  search(query, limit = 100) {
    ...
    return this.index
      .map((item) => {
        ...
        score += Math.max(
          0,
          10 - (Date.now() - new Date(item.modifiedAt)) / 86_400_000 / 30,
        );
        return {
          ...item,
          content: undefined,
          snippet: item.content
            ? safeSnippet(
                item.content,
                terms.find((term) => content.includes(term)) ?? raw,
              )
            : "",
          score,
        };
      ...
      .sort(
        (a, b) =>
          b.score - a.score || new Date(b.modifiedAt) - new Date(a.modifiedAt),
      )
  ...
  isAllowed(file, roots) {
    ...
    return roots.some(
      (root) =>
        ...
        within(path.resolve(root), resolved),
    );
  }
}
...
export async function fileHealth(settings) {
  ...
  const largest = walked.files
    .filter((item) => item.size >= settings.healthLargeFileBytes)
    .sort((a, b) => b.size - a.size)
    .slice(0, 100);
  const stale = walked.files
    .filter((item) => item.mtimeMs < staleBoundary)
    .sort((a, b) => a.mtimeMs - b.mtimeMs)
    .slice(0, 100);
  const bySize = new Map();
  for (const item of walked.files.filter(
    (item) => item.size > 0 && item.size <= 500 * 1024 * 1024,
  )) {
  ...
  for (const group of [...bySize.values()]
    ...
    for (const item of group) {
      ...
      try {
        ...
        const items = byHash.get(digest) ?? [];
        items.push(item);
        byHash.set(digest, items);
      } catch {
        /* reported by scan when possible */
      }
    }
    for (const [hash, items] of byHash)
      if (items.length > 1)
        duplicateGroups.push({
          hash,
          size: items[0].size,
          recoverableBytes: items[0].size * (items.length - 1),
  ...
  return {
    ...
    bytes: walked.files.reduce((sum, item) => sum + item.size, 0),
    limited: walked.limited,
    largest,
    stale,
